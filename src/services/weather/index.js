...
class Weather {
  getUrl (locationConfig: string): Promise<string> {
    if (locationConfig !== 'auto') {
      return Promise.resolve(provider.createUrlFromLocation(locationConfig));
    }
    return location.getPosition().then((position: Position) => provider.createUrlFromPosition(position));
  }

  fetch (locationConfig: string) {
